...
const tryRand = (n: number, max: number, maxPowerBall: number) => {
  ...
  array.push(Math.floor(Math.random() * maxPowerBall) + 1);
...
function generateLottoNumbers({
  ...
  return Array(numberOfTickets)
    .fill(0)
    .map(() => tryRand(lottoSize, maxRange, pr))
    .reduce((a, b) => {
      return [...a, ...b];
    }, []);
}
...
describe('Lottery contract', function () {
  ...
  let lotteryNftInstance: Contract;
  // Creating the instance and contract info for the prizePool contract
  let prizePoolInstance: Contract;
  // Creating the instance and contract info for the prizeReservePool contract
  let prizeReservePoolInstance: Contract;
  // Creating the instance and contract info for the taxCollection contract
  let taxCollectionInstance: Contract;
  // Creating the instance and contract info for the cake token contract
  let ironInstance: Contract;
  // Creating the instance and contract info for the timer contract
  let timerInstance: Contract;
  // Creating the instance and contract info for the mock rand gen
  let randGenInstance: Contract;
  // Creating the instance and contract of all the contracts needed to mock
  // the ChainLink contract ecosystem.
  let linkInstance: Contract;
  let mock_vrfCoordInstance: Contract;
  ...
  beforeEach(async () => {
    ...
    lotteryInstance = await ethers.getContract('Lottery');
    lotteryNftInstance = await ethers.getContract('Ticket');
    // prizePoolInstance = await ethers.getContract('PrizePool');
    prizeReservePoolInstance = await ethers.getContract('PrizeReservePool');
    // taxCollectionInstance = await ethers.getContract('TaxService');

    timerInstance = await ethers.getContract('Timer');
    randGenInstance = await ethers.getContract('RandomNumberGenerator');
    ironInstance = await ethers.getContract('MockIRON');
    linkInstance = await ethers.getContract('MockLink');
    mock_vrfCoordInstance = await ethers.getContract('MockVRFCoordinator');
    await ironInstance.connect(owner).transfer(prizeReservePoolInstance.address, parseUnits('100000', 18));
    await ironInstance.connect(owner).transfer(buyer.address, parseUnits('1000', 18));
    await ironInstance.connect(owner).transfer(alice.address, parseUnits('1000', 18));
  });
  ...
  describe('Creating a new lottery tests', function () {
    ...
    it('Nominal case', async function () {
      ...
      await expect(
        lotteryInstance.connect(owner).createNewLotto(currentTime.toString(), currentTime.add(1000).toString())
      )
    ...
    it('Invalid admin', async function () {
      ...
      await expect(
        lotteryInstance.connect(buyer).createNewLotto(currentTime.toString(), currentTime.add(100).toString())
    ...
    it('Invalid timestamps', async function () {
      ...
      await expect(
        lotteryInstance.connect(owner).createNewLotto(0, currentTime.add(1000).toString())
      ).to.be.revertedWith('Timestamps for lottery invalid');
      // Checking call reverts with correct error message
      await expect(lotteryInstance.connect(owner).createNewLotto(currentTime, currentTime)).to.be.revertedWith(
        'Timestamps for lottery invalid'
      );
    });
  });

  describe.only('Buying tickets tests', function () {
    /**
    ...
    beforeEach(async () => {
      ...
      let currentTime = await lotteryInstance.getCurrentTime();
      // Creating a new lottery
      await lotteryInstance.connect(owner).manualStartLotto(currentTime, currentTime.add(1000));
    });
    ...
    it('Cost per ticket', async function () {
      ...
      expect(totalPrice).to.eql(parseUnits('10', 18));
    });
    ...
    it('Batch buying 1 tickets', async function () {
      ...
      await ironInstance.connect(buyer).approve(lotteryInstance.address, price);
      // Batch buying tokens
      await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 1, ticketNumbers);
      // Testing results
      expect(price).to.eql(parseUnits('1', 18));
    });
    /**
    ...
    it('Batch buying 10 tickets', async function () {
      ...
      await ironInstance.connect(buyer).approve(lotteryInstance.address, price);
      // Batch buying tokens
      const balance = await ironInstance.balanceOf(buyer.address);
      await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 10, ticketNumbers);
      const after = await ironInstance.balanceOf(buyer.address);
      expect(balance.sub(after)).to.eql(parseUnits('10', 18));
    });
    /**
     * Tests the batch buying of fifty token
     */
    it('Batch buying 20 tickets', async function () {
      // Getting the price to buy
      ...
      const balance = await ironInstance.balanceOf(buyer.address);
      await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 20, ticketNumbers, {
        gasLimit: 12450000,
      });
      const after = await ironInstance.balanceOf(buyer.address);

      expect(balance.sub(after)).to.eql(parseUnits('20', 18));
    });
    /**
    ...
    it('Invalid chosen numbers', async function () {
      ...
      // Approving lotto to spend cost
      await ironInstance.connect(owner).approve(lotteryInstance.address, price);
      // Batch buying tokens
      await expect(lotteryInstance.connect(owner).batchBuyLottoTicket(1, 10, ticketNumbers)).to.be.revertedWith(
    ...
    it('Invalid buying time', async function () {
      ...
      let futureTime = currentTime.add(1000);
  ...
  //     let timeStamp = BigNumber.from(currentTime.toString());
  //     // Creating a new lottery
  ...
  //     await lotteryInstance.setCurrentTime(futureTime.toString());
  //     // Drawing the numbers
  //     let tx = await (await lotteryInstance.connect(owner).drawWinningNumbers(1, 1234)).wait();
  ...
  //       .connect(owner)
  //       .callBackWithRandomness(requestId, lotto.draw.random, randGenInstance.address);
  ...
  //     await lotteryInstance.setCurrentTime(futureTime.toString());
  //     // Drawing the numbers
  //     await expect(lotteryInstance.connect(buyer).drawWinningNumbers(1, 1234)).to.be.revertedWith(
  ...
  //     await expect(lotteryInstance.connect(owner).drawWinningNumbers(1, 1234)).to.be.revertedWith(
  ...
  //     await ironInstance.connect(buyer).mint(buyer.address, prices[2]);
  //     // Approving lotto to spend cost
  //     await ironInstance.connect(buyer).approve(lotteryInstance.address, prices[2]);
  //     // Generating chosen numbers for buy
  //     let ticketNumbers = generateLottoNumbers({
  //       numberOfTickets: 50,
  ...
  //     });
  //     // Batch buying tokens
  //     await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 50, ticketNumbers);
  //   });
  //   /**
  ...
  //     // Sending the buyer the needed amount of cake
  //     await ironInstance.connect(owner).transfer(buyer.address, prices[2]);
  //     // Approving lotto to spend cost
  //     await ironInstance.connect(buyer).approve(lotteryInstance.address, prices[2]);
  //     await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 1, lotto.newLotto.win.winningNumbersArr);
  ...
  //       .connect(owner)
  //       .callBackWithRandomness(requestId, lotto.draw.random, randGenInstance.address);
  //     let buyerCakeBalanceBefore = await ironInstance.balanceOf(buyer.address);

  //     // Getting the current block timestamp
  //     currentTime = await lotteryInstance.getCurrentTime();
  //     // Converting to a BigNumber for manipulation
  //     timeStamp = BigNumber.from(currentTime.toString());
  //     let futureEndTime = timeStamp.plus(lotto.newLotto.closeIncrease);
  //     // Setting the time forward
  //     await lotteryInstance.setCurrentTime(futureEndTime.toString());
  //     // Claiming winnings
  //     await lotteryInstance.connect(buyer).claimReward(1, userTicketIds[50].toString());
  //     let buyerCakeBalanceAfter = await ironInstance.balanceOf(buyer.address);
  //     // Tests
  //     assert.equal(buyerCakeBalanceBefore.toString(), 0, 'Buyer has cake balance before claiming');
  //     assert.equal(
  ...
  //     // Tests
  //     assert.equal(buyerCakeBalanceBefore.toString(), 0, 'Buyer has cake balance before claiming');
  //     assert.equal(buyerCakeBalanceAfter.toString(), 0, 'User won incorrect amount');
  //   });
  //   /**
  ...
  //     // Claiming winnings
  //     await expect(lotteryInstance.connect(buyer).claimReward(1, userTicketIds[25].toString())).to.be.revertedWith(
  ...
  //     await lotteryInstance.setCurrentTime(futureEndTime.toString());
  //     // Claiming winnings
  //     await expect(lotteryInstance.connect(owner).claimReward(1, userTicketIds[25].toString())).to.be.revertedWith(
  //       lotto.errors.invalid_claim_owner
  //     );
  //   });
  //   /**
  ...
  //     await lotteryInstance.setCurrentTime(futureEndTime.toString());
  //     // Claiming winnings
  //     await lotteryInstance.connect(buyer).claimReward(1, userTicketIds[25].toString());
  //     // Claiming winnings again
  //     await expect(lotteryInstance.connect(buyer).claimReward(1, userTicketIds[25].toString())).to.be.revertedWith(
  //       lotto.errors.invalid_claim_duplicate
  //     );
  //   });
  //   /**
  ...
  //     // Approving lotto to spend cost
  //     await ironInstance.connect(buyer).approve(lotteryInstance.address, prices[2]);
  //     await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 1, lotto.errorData.ticketNumbers);
  //     // Setting current time so that drawing is correct
  //     // Getting the current block timestamp
  //     let currentTime = await lotteryInstance.getCurrentTime();
  //     // Converting to a BigNumber for manipulation
  ...
  //     await expect(lotteryInstance.connect(buyer).claimReward(1, userTicketIds[50].toString())).to.be.revertedWith(
  //       lotto.errors.invalid_numbers_range
  //     );
  //   });
  ...
  //     // Approving lotto to spend cost
  //     await ironInstance.connect(buyer).approve(lotteryInstance.address, prices[2]);
  //     await lotteryInstance.connect(buyer).batchBuyLottoTicket(2, 1, lotto.newLotto.win.winningNumbersArr);
  //     // Setting current time so that drawing is correct
  //     // Getting the current block timestamp
  //     currentTime = await lotteryInstance.getCurrentTime();
  //     // Converting to a BigNumber for manipulation
  ...
  //     await lotteryInstance.setCurrentTime(futureEndTime.toString());
  //     // Claiming winnings with invalid numbers
  //     await expect(lotteryInstance.connect(buyer).claimReward(1, userTicketIds[0].toString())).to.be.revertedWith(
  //       lotto.errors.invalid_claim_lottery
  //     );
  //   });
  // });
  ...
  //     });
  //     // Batch buying tokens
  //     await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 49, ticketNumbers);
  //     // Getting the price to buy
  ...
  //     // Testing
  //     assert.equal(sizeOfLottery.toString(), lotto.setup.sizeOfLottery, 'Start size incorrect');
  //     assert.equal(sizeOfLotteryAfter.toString(), lotto.update.sizeOfLottery, 'Start incorrect after update');
  //   });
  //   /**
  ...
  //     let sizeOfLottery = await lotteryInstance.sizeOfLottery_();
  //     // Updating the size of the lottery
  //     await expect(lotteryInstance.updateSizeOfLottery(sizeOfLottery.toString())).to.be.revertedWith(
  //       lotto.errors.invalid_size_update_duplicate
  //     );
  //   });
  //   /**
  ...
  //     // Updating the size of the lottery
  //     await expect(lotteryInstance.connect(buyer).updateSizeOfLottery(lotto.update.sizeOfLottery)).to.be.revertedWith(
  //       lotto.errors.invalid_admin
  //     );
  //   });
  //   /**
  ...
  //     // Updating range
  //     await lotteryInstance.connect(owner).updateMaxRange(lotto.update.maxValidRange);
  //     // Getting the range after
  //     let maxRangeAfter = await lotteryInstance.maxValidRange_();
  //     // Testing
  //     assert.equal(maxRange.toString(), lotto.setup.maxValidRange, 'Max range incorrect before update');
  //     assert.equal(maxRangeAfter.toString(), lotto.update.maxValidRange, 'Max range incorrect after update');
  //   });
  //   /**
  ...
  //   it('Invalid update size of lottery (same as current)', async function () {
  //     // Updating range
  //     await expect(lotteryInstance.connect(owner).updateMaxRange(lotto.setup.maxValidRange)).to.be.revertedWith(
  //       lotto.errors.invalid_size_update_duplicate
  //     );
  //   });
  //   /**
  ...
  //   it('Invalid update size of lottery (non-owner)', async function () {
  //     // Updating the size of the lottery
  //     await expect(lotteryInstance.connect(buyer).updateMaxRange(lotto.setup.maxValidRange)).to.be.revertedWith(
  //       lotto.errors.invalid_admin
  //     );
  //   });
  ...
  //     // Testing
  //     assert.equal(bucketOne.toString(), lotto.setup.bucket.one, 'Bucket one incorrect');
  //     assert.equal(bucketOneAfter.toString(), lotto.update.bucket.one, 'Bucket one incorrect');
  //     assert.equal(bucketTwo.toString(), lotto.setup.bucket.two, 'Bucket two incorrect');
  //     assert.equal(bucketTwoAfter.toString(), lotto.update.bucket.two, 'Bucket two incorrect');
  //     assert.equal(disBucketOne.toString(), lotto.setup.bucketDiscount.one, 'Bucket one discount incorrect');
  //     assert.equal(disBucketOneAfter.toString(), lotto.update.bucketDiscount.one, 'Bucket one discount incorrect');
  //     assert.equal(disBucketTwo.toString(), lotto.setup.bucketDiscount.two, 'Bucket two discount incorrect');
  //     assert.equal(disBucketTwoAfter.toString(), lotto.update.bucketDiscount.two, 'Bucket two discount incorrect');
  //     assert.equal(disBucketThree.toString(), lotto.setup.bucketDiscount.three, 'Bucket three discount incorrect');
  //     assert.equal(
  ...
  //     let ownerCakeBalance = await ironInstance.balanceOf(owner.address);
  //     let lotteryCakeBalance = await ironInstance.balanceOf(lotteryInstance.address);
  //     await lotteryInstance.connect(owner).withdrawCake(lotteryCakeBalance);
  //     let ownerCakeBalanceAfter = await ironInstance.balanceOf(owner.address);
  //     let lotteryCakeBalanceAfter = await ironInstance.balanceOf(lotteryInstance.address);
  //     // Tests
  //     assert.notEqual(
  ...
  //     );
  //     assert.equal(lotteryCakeBalanceAfter.toString(), 0, 'Lottery has not been drained');
  //   });

  //   it('Invalid withdraw excess cake (non owner)', async function () {
  //     let lotteryCakeBalance = await ironInstance.balanceOf(lotteryInstance.address);
  //     await expect(lotteryInstance.connect(buyer).withdrawCake(lotteryCakeBalance)).to.be.revertedWith(
  //       lotto.errors.invalid_admin
  //     );
  //   });
  // });
  ...
  //     let pricesBucketOne = await lotteryInstance.costToBuyTicketsWithDiscount(1, 10);
  //     let pricesBucketTwo = await lotteryInstance.costToBuyTicketsWithDiscount(1, 35);
  //     let pricesBucketThree = await lotteryInstance.costToBuyTicketsWithDiscount(1, 51);
  //     assert.equal(pricesBucketOne[0].toString(), lotto.discount.ten.cost, 'Cost for buy of 10 incorrect');
  //     assert.equal(pricesBucketOne[1].toString(), lotto.discount.ten.discount, 'Discount for buy of 10 incorrect');
  //     assert.equal(
  //       pricesBucketOne[2].toString(),
  //       lotto.discount.ten.discountCost,
  //       'Discount cost for buy of 10 incorrect'
  //     );
  //     assert.equal(pricesBucketTwo[0].toString(), lotto.discount.thirty_five.cost, 'Cost for buy of 35 incorrect');
  //     assert.equal(
  ...
  //     );
  //     assert.equal(pricesBucketThree[0].toString(), lotto.discount.fifty_one.cost, 'Cost for buy of 55 incorrect');
  //     assert.equal(
  ...
  //     );
  //     assert.equal(lottoInfo.prizePoolInCake.toString(), lotto.newLotto.prize.toString(), 'Invalid prize pool');
  //     assert.equal(lottoInfo.costPerTicket.toString(), lotto.newLotto.cost, 'Invalid cost per token');
  //     assert.equal(lottoInfo.startingTimestamp.toString(), timeStamp.toString(), 'Invalid starting time');
  //     assert.equal(
  ...
  //     let tickets = await lotteryNftInstance.getUserTicketsPagination(buyer.address, 1, 0, 50);
  //     let ticketsTwo = await lotteryNftInstance.getUserTicketsPagination(buyer.address, 1, 50, 50);
  //     let userTickets = await lotteryNftInstance.getUserTickets(1, buyer.address);
